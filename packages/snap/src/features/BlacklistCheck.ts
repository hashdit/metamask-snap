...
export async function callDomainSecurity(transactionUrl?: any, apiKey?: any): Promise<Component[] | null> {
	if (!transactionUrl) {
		return null;
	}
	...
	try {
		...
		if (resp.code === "0" && resp.status === "ok" && resp.data.has_result) {
			...
			const contentArray: Component[] = [
				heading('Website Security Checkerrr'),
            ...
            if (riskLevel === 0 ) {
                contentArray.push(
                    text('This website appears to be safe based on our security analysis.')
                );
            } else {
                contentArray.push(
                    text('Please exercise caution when interacting with this website.')
                );
            }
