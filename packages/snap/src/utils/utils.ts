...
export async function verifyContractAndFunction(transaction: any, chainId:string, apiKey:string) {
    ...
    if (chain) {
		// Get verify result
        const isDestinationVerifiedResult = await isDestinationVerified(transaction.to, chain, apiKey);
    ...
    try {
        const response = await fetch(`https://www.4byte.directory/api/v1/signatures/?hex_signature=${functionSelector}`);
...
async function isDestinationVerified(contractAddress: string, chainId:string, apiKey:string) {
    ...
    try {
        ...
        if (status === "ok" && data) {
            return !(data.address_type === "Contract" && data.verification_status === "unverified");
        }
    } catch (error) {
        console.error("Fetch error:", error);
    }
